...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Sales Portal</h2>
    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Welcome back, {{ explode(' ', trim(auth()->user()->name))[0] }}. Add leads and convert clients.</p>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
    ...
    <a href="{{ route('clients.index') }}" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 flex items-start gap-4 hover:border-emerald-300 hover:shadow-md transition-all group">
        ...
        <div>
            <h3 class="font-bold text-lg text-slate-800 dark:text-white">Leads & Clients</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Add leads and convert them.</p>
        </div>
    </a>
