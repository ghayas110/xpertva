...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <form action="{{ route('attendance.manage') }}" method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
            <div class="relative">
                <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                <input type="text" id="employeeSearchInput" placeholder="Search employee..." oninput="filterEmployeeSelect()" class="pl-9 pr-3 py-2 text-sm border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 rounded text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[200px]">
            </div>
            <select id="attendanceEmployeeSelect" name="user_id" onchange="this.form.submit()" class="shadow-sm border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white rounded py-2 px-3 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[250px]">
                <option value="">-- Select Employee --</option>
                @foreach($users as $userOption)
                    <option value="{{ $userOption->id }}" {{ ($selectedUser && $selectedUser->id === $userOption->id) ? 'selected' : '' }}
                        data-name="{{ strtolower($userOption->name) }} {{ strtolower($userOption->role ?? '') }}">
